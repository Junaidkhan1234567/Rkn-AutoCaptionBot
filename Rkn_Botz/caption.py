...
async def set_caption(client, message):
    ...
    if existing:
        await rkn_botz._channels_collection.update_one({"channelId": channel_id}, {"$set": {"caption": caption}})
    else:
        await rkn_botz._channels_collection.insert_one({"channelId": channel_id, "caption": caption})
...
async def auto_caption(client, message):
    try:
        ...
        # Caption template
        caption_template = channel_data.get("caption") if channel_data else Rkn_Botz.CAPTION
        ...
        replaced_caption = replaced_caption.replace("{year}", str(detect_year(file_name)))
        replaced_caption = replaced_caption.replace("{season}", str(detect_season(file_name)))
        replaced_caption = replaced_caption.replace("{episode}", str(detect_episode(file_name)))
        replaced_caption = replaced_caption.replace("{quality}", str(detect_quality(file_name)))
        replaced_caption = replaced_caption.replace("{language}", str(detect_language(file_name)))
        replaced_caption = replaced_caption.replace("{size}", str(convert_size(file_size)))
        ...
        print(f"Error in auto_caption: {e}")
